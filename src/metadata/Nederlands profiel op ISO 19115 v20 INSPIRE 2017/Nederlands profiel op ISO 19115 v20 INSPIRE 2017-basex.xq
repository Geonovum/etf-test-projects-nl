   ...
   for $pattern in $sch/*/iso:pattern
      ...
      <etf:TestSteps>
      {
        for $rule in $pattern/iso:rule
          ...
          <etf:Label>{if ($rule/@etf_name) then data($rule/@etf_name) else data($ruleId)}</etf:Label>
          <etf:Description/>
          <etf:Assertions>
          {
            for $assert in $rule/iso:assert
              ...
              <etf:Label>{if ($assert/@etf_name) then data($assert/@etf_name) else data($assertId)}</etf:Label>
              <etf:Properties>
                <ii:Items>
                 <ii:Item name="RequirementReference">
                  <ii:value xsi:type="xs:string" xmlns:xs="http://www.w3.org/2001/XMLSchema"
             ...
             <ii:Item name="Description">
              <ii:value xsi:type="xs:string" xmlns:xs="http://www.w3.org/2001/XMLSchema"
               xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">Not available</ii:value>
                ...
                <ii:Item name="Reference">
                    <ii:value xsi:type="xs:string" xmlns:xs="http://www.w3.org/2001/XMLSchema"
                        xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance">{if ($assertOrReport/@see) then data($assertOrReport/@see) else 'Not available'}</ii:value>
                </ii:Item>
            </ii:Items>
        </etf:Properties>
        <etf:SubRequirements/>
