...
declare function local:test($db as document-node()*, $ets as element()*, $testQuery as xs:string) as element()
{
let $query := $testQuery || 
...
return try {
  xquery:eval($query, map {'validationErrors': $validationErrors, 'db': $db, 'files_to_test': $files_to_test, 'tests_to_execute': $tests_to_execute, 'limitErrors': $limitErrors, 'testObjectId': $testObjectId, 'logFile': $logFile })
} catch * {
let $text := '[' || $err:code || '] ' || $err:description || ' 
' || $err:module || ' (' || $err:line-number || '/' || $err:column-number || ')'
let $message := 
...
declare variable $projDir external := "/Users/portele/Documents/Dropbox/ETF/github/etf-ets-repository/example";
...
declare variable $dbBaseName external := "sch-test";
declare variable $dbCount external := 1;
declare variable $etsFile external := $projDir || file:dir-separator() || "3b_schematrontest" || file:dir-separator() || "schematron-bsxets.xml"; 
(: Project internals :)
declare variable $testQueryFile := "schematron-query-prolog.xq";
...
let $namespaces := $ets/etf:ParameterList/etf:parameter[@name="xml_namespaces"]/etf:defaultValue/text()

let $res := local:test($db, $ets, $namespaces || $testQuery || $startlog)
